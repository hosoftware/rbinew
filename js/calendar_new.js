...
//css which gives flat view

// end css which gives flat view 
...
function selected(cal, date) {
	 ...
	 /* Check whether future date */
	 
	 var dd = new Date();
	 var dd_day = dd.getDate();
	 var dd_month = dd.getMonth()+1;
	 if(dd_month<10)
	 	dd_month_disp = "0"+dd_month;
	 else
	 	dd_month_disp = dd_month;
	 var dd_year = dd.getFullYear();
	 var correct = dd_month+"/"+dd_day+"/"+dd_year;
	 var today_display = dd_day+"-"+dd_month_disp+"-"+dd_year;
	 
	 var todays=Date.parse(correct);
	 
	 var demo = date;
	 var arr = demo.split("-");
	 var selected = arr[1]+"/"+arr[0]+"/"+arr[2];
	 	
	 var selected=Date.parse(selected);
	 
	
		cal.sel.value = date;
	
if((from_page=='jobdiary')&&(cal.sel.id=="date_from"))
window.location.href='jobdiary.php?date='+date;
	 
	 /* End of check whether future date */
	 
}
/*** Function to display the time in and time out ***/
function display_time(date)
{
	...
	var request_save = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
		...
		request_save.open("POST", "jobdiary_ajax.php", true);
	...
	request_save.onreadystatechange = function(){
		...
		{
			document.getElementById('work_sheet').innerHTML = request_save.responseText;
	...
	request_save.send('date='+date_get);
}
/*** End of Function to display the time in and time out ***/
...
if (calendar != null) {
    ...
    var cal = new Calendar(false, null, selected);
