...
class struct_option {
		...
		if(!empty($_REQUEST['project_id'])) {
			...
			$sql_table = "CREATE TABLE IF NOT EXISTS  ".$this->project_no."_struct_option (
						 id int(10) NOT NULL AUTO_INCREMENT,
							 ...
							 non_severe_corr tinyint(1) NOT NULL DEFAULT '0',
							 install_date varchar(255) DEFAULT NULL,
							 od_size double(20,3) DEFAULT '0.000',
							 nde varchar(255) DEFAULT NULL,
							 pressure double(20,0) DEFAULT '0',
							 allowable_stress double(20,2) DEFAULT '0.00',
							 ...
							 thickness1 double(20,1) DEFAULT '0.0',
							 thickness2 double(20,1) DEFAULT '0.0',
							 thickness3 double(20,1) DEFAULT '0.0',
							 thickness4 double(20,1) DEFAULT '0.0',
							 thickness5 double(20,1) DEFAULT '0.0',
							 thickness6 double(20,1) DEFAULT '0.0',
							 thickness7 double(20,1) DEFAULT '0.0',
							 thickness8 double(20,1) DEFAULT '0.0',
							 corrosion_rate double(20,2) DEFAULT '0.00',
							 design_thickness double(20,2) DEFAULT '0.00',
		...
		if(!empty($_REQUEST['doAction'])) {
			switch($_REQUEST['doAction']) {
				case 'Add_New_Project':
					...
					break;
				case"AddProject":
					$sql_ins = "INSERT IGNORE INTO rbi_project_details(	project,jobno,location,	tank,reference,incharge,year,date_of_sarvey,sarveydate)
					...
					$sql_table = "CREATE TABLE IF NOT EXISTS  ".$maxno."_struct_option (
						 ...
						 install_date varchar(255) DEFAULT NULL,
						 od_size double(20,3) DEFAULT '0.00',
						 nde varchar(255) DEFAULT NULL,
						 pressure double(20,0) DEFAULT '0.00',
						 allowable_stress double(20,2) DEFAULT '0.00',
						 conditions varchar(255) DEFAULT NULL,
						 schedules varchar(255) DEFAULT NULL,
						 temperature double(20,2) DEFAULT '0.00',
					...
					break;
				case 'UpdateProject':
					 $sql_update = "UPDATE IGNORE rbi_project_details
	...
	//	$struct_option = "";
		while($row = mysqli_fetch_assoc($result)) {
			...
			if(!is_numeric($row['wall_tickness'])|| $row['wall_tickness'] =='0.00') {
				$wall_tickness=0;
			}
			else $wall_tickness=$row['wall_tickness'];
			if(!is_numeric($mean_thickness1) || $mean_thickness1=='0.00') {
				$mean_thickness1=0;
			}

			if(!empty($wall_tickness) && !empty($mean_thickness1)) {
			$row['diminution1'] = round((($wall_tickness-$mean_thickness1)/100)*1000,2);
			}
			else {
				$row['diminution1'] ='-';
			}//round((($wall_tickness-$mean_thickness1)/100)*1000,2);
			...
			if(!empty($pressure) && $pressure>0 && empty($row['non_severe_corr'])) {
				//$row['design_thickness']= round(((($od_size)-(2*$row['corrosion']))/2)/(1/((1.155* $pressure)/$allowable_stress))-1,2);
				$value = ((1.155* $pressure)/$allowable_stress);
				$exponent = exp(-1*$value);
				$row['design_thickness']= (($od_size-(2*($row['corrosion']/25.4)))/2) * (1-$exponent);
				//((($od_size)-(2*($row['corrosion']/25.4)))/2);
				//round(((($od_size)-(2*($row['corrosion']/25.4)))/2)/(1-$exponent),2);
			}
			if(($row['temperature'])<=0) $row['temperature']=0;
	...
	function insertImageDetails() {
		 $sql_ins = "INSERT IGNORE INTO ".$this->project_no."_image_details (project_id,title1,description1,title2,description2,title3,description3,title4,description4,rating1,observations1, recommendations1,title5,description5,title6,description6,title7,description7,title8,description8,rating2,observations2,recommendations2
	...
	function updateImageDetails() {
		 $upd_sql = "UPDATE IGNORE ".$this->project_no."_image_details
